...
from typing import Tuple, Union, List, MutableMapping, Any, Iterator
...
class _KVDataProxy(MutableMapping[str, Any]):
    ...
    def __iter__(self) -> Iterator:
        return iter([a[1] for a in self.items()])
...
def parse_kv(buffer: IBuffer, file_name: str) -> _KVDataProxy:
    parser = ValveKeyValueParser(buffer_and_name=(buffer.read().decode('utf8'), file_name), self_recover=True)
    parser.parse()
    return parser.tree
