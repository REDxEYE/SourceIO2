...
from SourceIO2.source2.data_types.blocks import BaseBlock, ResourceExternalReferenceList
from SourceIO2.source2.resource_types.resource import ICompiledResource
from SourceIO2.utils import IBuffer
from SourceIO2.source2.data_types.keyvalues3.binary_keyvalues import (
    TypedArray, Object,
    UInt64, Int64,
    UInt32, Int32,
    Double,
...
from .struct import Struct, KeyValueDataType, StructMember
...
class ResourceIntrospectionManifest(BaseBlock):
    ...
    def from_file(cls, buffer: IBuffer, resource: ICompiledResource):
        ...
        with buffer.read_from_offset(struct_offset):
            for i in range(struct_count):
                structure = Struct.from_file(buffer)
        ...
        with buffer.read_from_offset(enum_offset):
            for i in range(enum_count):
                enum = Enum.from_file(buffer)
                self._s2p_enum[enum.name] = enum
                self._i2p_enum[enum.id] = enum
                self._enums.append(enum)
        return self

    def read_struct(self, buffer: IBuffer, top_struct: Struct) -> Union[NullObject, Object]:
        data = Object()
    ...
    def _read_struct(self, buffer: IBuffer, member: StructMember):
        return self.read_struct(buffer, self.struct_by_id(member.data_type))

    def _read_enum(self, buffer: IBuffer, member: StructMember):
        return String(self.enum_by_id(member.data_type).values[buffer.read_uint32()].name)

    def _read_ex_ref(self, buffer: IBuffer, member: StructMember):
    ...
    def _read_string(self, buffer: IBuffer, member: StructMember):
        offset = buffer.read_relative_offset32()
        if offset == buffer.tell() - 4:
            return String('')
        with buffer.read_from_offset(offset):
    ...
    def _read_ubyte(self, buffer: IBuffer, member: StructMember):
        return UInt32(buffer.read_uint8())

    def _read_byte(self, buffer: IBuffer, member: StructMember):
        return Int32(buffer.read_int8())

    def _read_ushort(self, buffer: IBuffer, member: StructMember):
        return UInt32(buffer.read_uint16())

    def _read_short(self, buffer: IBuffer, member: StructMember):
        return Int32(buffer.read_int16())

    def _read_uint32(self, buffer: IBuffer, member: StructMember):
        return UInt32(buffer.read_uint32())

    def _read_int32(self, buffer: IBuffer, member: StructMember):
        return Int32(buffer.read_int32())

    def _read_uint64(self, buffer: IBuffer, member: StructMember):
        return UInt64(buffer.read_uint64())

    def _read_int64(self, buffer: IBuffer, member: StructMember):
        return Int64(buffer.read_int64())

    def _read_float(self, buffer: IBuffer, member: StructMember):
        return Double(buffer.read_float())

    def _read_vector2(self, buffer: IBuffer, member: StructMember):
        return TypedArray(KV3Type.DOUBLE, KV3TypeFlag.NONE,
                          [Double(buffer.read_float()), Double(buffer.read_float())])

    def _read_vector3(self, buffer: IBuffer, member: StructMember):
        return TypedArray(KV3Type.DOUBLE, KV3TypeFlag.NONE,
                          [Double(buffer.read_float()), Double(buffer.read_float()), Double(buffer.read_float())])

    def _read_vector4(self, buffer: IBuffer, member: StructMember):
        return TypedArray(KV3Type.DOUBLE, KV3TypeFlag.NONE,
                          [Double(buffer.read_float()) for _ in range(4)])

    def _read_color(self, buffer: IBuffer, member: StructMember):
        return TypedArray(KV3Type.DOUBLE, KV3TypeFlag.NONE,
                          [Double(buffer.read_uint8() / 255) for _ in range(4)])

    def _read_bool(self, buffer: IBuffer, member: StructMember):
        return Bool(buffer.read_uint8() == 1)

    def _read_mat34(self, buffer: IBuffer, member: StructMember):
        return TypedArray(KV3Type.DOUBLE, KV3TypeFlag.NONE,
                          [Double(buffer.read_float()) for _ in range(12)])

    def _read_ctrans(self, buffer: IBuffer, member: StructMember):
        return TypedArray(KV3Type.DOUBLE, KV3TypeFlag.NONE,
